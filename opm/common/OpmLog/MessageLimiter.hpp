...
MessageLimiter(const int tag_limit, const std::map<int64_t,
               int> category_limits)
  : tag_limit_(tag_limit < 0 ? NoLimit : tag_limit),
    tag_counts_(),
...
/// function).
enum class Response {
  PrintMessage, JustOverTagLimit, JustOverCategoryLimit,
  OverTagLimit, OverCategoryLimit
};

  /// If (tag count == tag limit + 1) for the passed tag,
  /// respond JustOverTagLimit.
  ...
  /// If (category count == category limit + 1) for the passed messageMask,
  /// respond JustOverCategoryLimit.
  /// If (category count > category limit + 1), respond OverCategoryLimit.
  /// If (category count <= category limit), or there is no limit for that
  /// category, respond PrintMessage.
 ...
 private:
  ...
  Response countBasedResponseCategory(const int count,
                                      const int64_t messageMask) const {
