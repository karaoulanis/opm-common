...
void Logger::addTaggedMessage(int64_t messageType, const std::string& tag,
                              const std::string& message) const {
  if ((m_enabledTypes & messageType) == 0) {
      throw std::invalid_argument(
        "Tried to issue message with unrecognized message ID");
  }
...
void Logger::addBackend(const std::string& name,
...
void Logger::addMessageType(int64_t messageType,
