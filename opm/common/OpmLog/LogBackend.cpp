...
void LogBackend::setMessageFormatter(
...
void LogBackend::addTaggedMessage(int64_t messageType,
...
bool LogBackend::includeMessage(int64_t messageFlag,
                                const std::string& messageTag) {
  // Check mask
  const bool included = ((messageFlag & m_mask) == messageFlag)
                      && (messageFlag > 0);
  if (!included) {
    return false;
  }
  ...
  if (res == MessageLimiter::Response::JustOverCategoryLimit) {
    ...
    std::string msg = "Message limit reached for message category: "
                    + prefix.substr(0, prefix.size()-2);
    addMessageUnconditionally(messageFlag, msg);
  }
  return res == MessageLimiter::Response::PrintMessage;
}

std::string LogBackend::formatMessage(int64_t messageFlag,
                                      const std::string& message) {
  if (m_formatter) {
